...
export function AdminUserDetailPage() {
  ...
  useEffect(() => {
    ...
    fetchUser(userId)
      ...
      .catch((err) => {
        setError(err.message || (isDe ? 'Benutzerdaten konnten nicht geladen werden' : 'Failed to load user details'))
      })
      .finally(() => {
        setLoading(false)
      })
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPassword('')
      setEditing(false)
      setMessage({ type: 'success', text: isDe ? 'Benutzer erfolgreich aktualisiert!' : 'User updated successfully!' })
      setTimeout(() => setMessage(null), 3000)
    } catch (err: any) {
      setMessage({ type: 'error', text: err.message || (isDe ? 'Benutzer konnte nicht aktualisiert werden' : 'Failed to update user') })
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: err.message || (isDe ? 'Benutzer konnte nicht gelöscht werden' : 'Failed to delete user') })
      setDeleting(false)
      setDeleteModalOpen(false)
    }
  }
  ...
  const t = {
    loading: isDe ? 'Benutzerdaten werden geladen...' : 'Loading user details...',
    notFound: isDe ? 'Benutzer nicht gefunden' : 'User not found',
    backToUsers: isDe ? 'Zurück zur Übersicht' : 'Back to Users',
    joined: isDe ? 'Registriert' : 'Joined',
    userIdLabel: isDe ? 'Benutzer-ID' : 'User ID',
    cancelEditing: isDe ? 'Bearbeiten abbrechen' : 'Cancel Editing',
    editAccount: isDe ? 'Konto bearbeiten' : 'Edit Account',
    deleteAccount: isDe ? 'Konto löschen' : 'Delete Account',
    userAnalytics: isDe ? 'Benutzer-Analysen' : 'User Analytics',
    contracts: isDe ? 'Verträge' : 'Contracts',
    contractsSub: isDe ? 'Kaufverträge' : 'Purchase agreements',
    invoices: isDe ? 'Rechnungen' : 'Invoices',
    invoicesSub: isDe ? 'Rechnungsverlauf' : 'Billing history',
    repairOrders: isDe ? 'Reparaturaufträge' : 'Repair Orders',
    repairOrdersSub: isDe ? 'Gerätereparaturen' : 'Device repairs',
    modifySettings: isDe ? 'Kontoeinstellungen ändern' : 'Modify Account Settings',
    displayName: isDe ? 'Anzeigename' : 'Display Name',
    emailAddress: isDe ? 'E-Mail-Adresse' : 'Email Address',
    changePassword: isDe ? 'Passwort ändern' : 'Change Password',
    passwordPlaceholder: isDe ? 'Leer lassen, um aktuelles Passwort zu behalten' : 'Leave blank to keep current password',
    accountStatus: isDe ? 'Kontostatus' : 'Account Status',
    selfDeactivateWarning: isDe ? 'Sie können Ihr eigenes Administrator-Konto nicht deaktivieren.' : 'You cannot deactivate your own administrative account.',
    active: isDe ? 'Aktiv' : 'Active',
    inactive: isDe ? 'Inaktiv' : 'Inactive',
    accountRole: isDe ? 'Kontorolle' : 'Account Role',
    selfRoleWarning: isDe ? 'Sie können Ihre eigene Administrator-Rolle nicht ändern.' : 'You cannot demote or change your own administrative role.',
    staff: isDe ? 'Mitarbeiter' : 'Staff',
    admin: isDe ? 'Admin' : 'Admin',
    savingChanges: isDe ? 'Änderungen werden gespeichert...' : 'Saving Changes...',
    saveSettingsBtn: isDe ? 'Benutzereinstellungen speichern' : 'Save User Settings',
    cancelBtn: isDe ? 'Abbrechen' : 'Cancel',
    overviewTitle: isDe ? 'Übersicht & Einblicke' : 'Overview & Insights',
    overviewSub: isDe ? 'Verwenden Sie die linke Spalte, um Kontokonfigurationen zu bearbeiten oder Verträge, Rechnungen und Reparaturaufträge dieses Benutzers zu durchsuchen.' : 'Use the left-hand column to edit account configurations or browse contracts, invoices, and repair orders generated by this user.',
    deleteTitle: isDe ? 'Benutzerkonto löschen' : 'Delete User Account',
    deleteWarning: (name: string) => isDe
      ? `Sind Sie sich absolut sicher, dass Sie das Konto von "${name}" löschen möchten? Diese Aktion ist dauerhaft und löscht auch alle damit verbundenen Verträge, Rechnungen und Reparaturaufträge.`
      : `Are you absolutely sure you want to delete "${name}"? This action is permanent and will cascade, hard-deleting all associated contracts, invoices, and repair orders.`,
    keepAccountBtn: isDe ? 'Nein, Konto behalten' : 'No, Keep Account',
    deleteAccountBtn: isDe ? 'Ja, Konto löschen' : 'Yes, Delete Account',
    deletingBtn: isDe ? 'Löschen...' : 'Deleting...',
  }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center px-4 text-center">
        <div>
          ...
          <p className="mt-3 text-sm font-medium text-red-300">{error || t.notFound}</p>
          <button
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="rounded-2xl border border-white/[0.06] bg-gradient-to-br from-white/[0.04] to-white/[0.01] p-6 shadow-xl">
            ...
            <div className="mt-6 space-y-2">
              ...
              >
                {editing ? t.cancelEditing : t.editAccount}
              </button>
              {!isSelf && (
                <button
          ...
          <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6 space-y-4">
            <h3 className="text-xs font-bold uppercase tracking-widest text-white/30">{t.userAnalytics}</h3>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-white/80">{t.contracts}</p>
                  <p className="text-[10px] text-white/30">{t.contractsSub}</p>
                </div>
              </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-white/80">{t.invoices}</p>
                  <p className="text-[10px] text-white/30">{t.invoicesSub}</p>
                </div>
              </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-white/80">{t.repairOrders}</p>
                  <p className="text-[10px] text-white/30">{t.repairOrdersSub}</p>
                </div>
              </div>
        ...
        <div className="lg:col-span-2">
          {editing ? (
            <div className="rounded-2xl border border-white/[0.06] bg-gradient-to-br from-white/[0.04] to-white/[0.01] p-8 shadow-xl">
              <h3 className="text-base font-bold text-white mb-6">{t.modifySettings}</h3>
              <form onSubmit={handleUpdate} className="space-y-5">
                ...
                <div className="pt-4 flex gap-3">
                  ...
                  >
                    {saving ? t.savingChanges : t.saveSettingsBtn}
                  </button>
                  <button
                    type="button"
                    onClick={() => {
            ...
            <div className="rounded-2xl border border-white/[0.06] bg-white/[0.01] p-8 text-center flex flex-col items-center justify-center h-full min-h-[350px]">
              ...
              <h3 className="text-base font-bold text-white/60">{t.overviewTitle}</h3>
      ...
      {deleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="w-full max-w-md rounded-2xl border border-red-500/20 bg-gray-900 p-6 shadow-2xl animate-scale-in">
            ...
            <h3 className="text-base font-bold text-white">{t.deleteTitle}</h3>
            <p className="mt-2 text-xs leading-relaxed text-white/50">
              {t.deleteWarning(user.name)}
            </p>
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              >
                {deleting ? t.deletingBtn : t.deleteAccountBtn}
              </button>
            </div>
          </div>
        </div>
