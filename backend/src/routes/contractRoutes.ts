...
import { draftContractSchema, searchContractsSchema } from "../validators/contractValidators.js";
...
contractRouter.get("/search", validate({ query: searchContractsSchema }), asyncHandler(contractController.search));
contractRouter.get("/validate-identifiers", asyncHandler(contractController.validateIdentifiers));
contractRouter.post("/draft", validate({ body: draftContractSchema }), asyncHandler(contractController.createDraft));
contractRouter.get("/:id", asyncHandler(contractController.getContract));
contractRouter.patch("/:id/draft", validate({ body: draftContractSchema }), asyncHandler(contractController.updateDraft));
